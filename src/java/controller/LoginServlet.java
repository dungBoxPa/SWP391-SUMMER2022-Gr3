...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        Account acc = d.checkAccount(email, password);
        try ( PrintWriter out = response.getWriter()) {
            if (acc != null && acc.getRole_id() == 3) {
                ClassDAO classDao = new ClassDAO();
                List<Account> list = d.getAllTeacherInfor();
                for (Account account : list) {
                    if (account.getEmail().equals(email) && account.getPassword().equals(password)) {
                        Kinder_Class kc = classDao.getTeacherClass(account.getAccount_id());
                ...
                List<Account> list = d.getAllParentInfor();
                for (Account account : list) {
                    if (account.getEmail().equals(email) && account.getPassword().equals(password)) {
                       session.setAttribute("parent", account);
